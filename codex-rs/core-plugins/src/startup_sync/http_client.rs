...
//! preserve the legacy `codex_login::default_client::build_reqwest_client()` behavior: invalid
//! custom-CA configuration is logged and falls back to a normal reqwest client instead of making
//! HTTP sync fail as well.
//!
...
impl StartupSyncHttpClient {
    pub(super) fn new(http_client_factory: &HttpClientFactory) -> Self {
        match http_client_factory.outbound_proxy_policy() {
            OutboundProxyPolicy::ReqwestDefault => Self::Default(build_reqwest_client()),
...
pub(super) enum StartupSyncRequestBuilder {
    Default(reqwest::RequestBuilder),
...
impl StartupSyncRequestBuilder {
    ...
    pub(super) async fn send(self) -> Result<reqwest::Response, String> {
        match self {
