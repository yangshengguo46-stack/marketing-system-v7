...
fn signed_usage_rows(side: &SyntheticSignedUsage) -> anyhow::Result<Vec<SyntheticResponseUsage>> {
    anyhow::ensure!(
        side.sha256 == sha256(&side.bytes),
        "response usage side commitment differs"
    );
    ...
    anyhow::ensure!(
        !rows.is_empty()
            && rows.iter().all(|row| !row.response_id.is_empty())
            && !rows
                .windows(2)
                .any(|pair| pair[0].response_id == pair[1].response_id),
        "response usage side is not a unique nonempty multiset"
    );
    Ok(rows)
}
...
impl SyntheticAuthorityBoundary for std::cell::RefCell<SyntheticCostBoundary> {
    ...
    fn validate_usage(&self, data: &LiveAuthorityData) -> anyhow::Result<()> {
        for (arm, sides) in data.arms.iter().zip(&self.borrow().response_usage) {
            let broker = signed_usage_rows(&sides[0])?;
            let app_server = signed_usage_rows(&sides[1])?;
            anyhow::ensure!(
                broker == app_server,
                "broker/App Server response usage multiset differs"
            );
            let mut total = [0_i64; 6];
            for row in &broker {
                for (sum, value) in total.iter_mut().zip(usage_parts(&row.usage)) {
                    *sum = sum
                        .checked_add(value)
                        .ok_or_else(|| anyhow::anyhow!("response usage sum overflow"))?;
                }
            }
            anyhow::ensure!(
                total == usage_parts(&arm.usage),
                "response usage multiset aggregate differs"
            );
        }
        Ok(())
    }
    ...
    fn publish_receipt(
        ...
        anyhow::ensure!(
            state.published_receipt.is_none(),
            "synthetic receipt sink is already occupied"
        );
...
fn cost_authority_json_bytes_canonicalizes_equivalent_source_order_after_edit() {
    let first = json_bytes(br#"{"\uE000":1,"\uD800\uDC00":2}"#, |value| {
        value["m"] = Value::String("changed".into())
    });
    let second = json_bytes(br#"{"\uD800\uDC00":2,"\uE000":1}"#, |value| {
        value["m"] = Value::String("changed".into())
    });
...
fn mode_evidence(input: &SyntheticLiveCostAuthority) -> ModeEvidence {
    ...
    ModeEvidence::Live {
        ...
        provider_endpoint_commitment: input.data.provider_endpoint_commitment.clone().unwrap(),
...
fn resign_attestation(input: &mut SyntheticLiveCostAuthority) {
    input_state(input).borrow_mut().documents[ATTESTATION] =
        serde_json::to_vec(&input.data.attestation).unwrap();
...
fn signed_usage(response_id: &str, usage: &Usage) -> SyntheticSignedUsage {
    let bytes = serde_json::to_vec(&vec![SyntheticResponseUsage {
        response_id: response_id.into(),
        usage: usage.clone(),
    }])
    .unwrap();
    SyntheticSignedUsage {
        sha256: sha256(&bytes),
        bytes,
    }
}
...
fn synthetic_input() -> SyntheticLiveCostAuthority {
    synthetic_input_with_order([EvaluationCondition::Generic, EvaluationCondition::Candidate])
}

fn synthetic_input_with_order(conditions: [EvaluationCondition; 2]) -> SyntheticLiveCostAuthority {
    let effective =
        |value: &mut Value| value["effectiveAt"] = Value::String("2026-08-30T09:00:00.000Z".into());
    let rate_card = json_bytes(
        include_bytes!("../tests/fixtures/contracts/06b1/provider-rate-card.canonical.json"),
        effective,
    );
    let billing_policy = json_bytes(
        include_bytes!("../tests/fixtures/contracts/06b1/billing-policy.canonical.json"),
        effective,
    );
    let fx_policy = json_bytes(
    ...
    let mut attestation: crate::NativeHeldOutAttestation = serde_json::from_slice(include_bytes!(
    ...
    attestation.approved_total_fen = 800;
    attestation.approved_per_run_fen = 400;
    attestation.max_provider_request_attempts_per_run = 2;
    attestation.max_total_tokens_per_run = 1_000;
    attestation.max_elapsed_seconds_per_run = 60;
    attestation.max_output_tokens_per_request = 4_096;
    attestation.provider_role = "targetVolcengine".into();
    attestation.target_provider_model_evidence_commitment = hex('3');
    let pair_id = hex('c');
    let frozen_sha256 = hex('1');
    let execution_context_sha256 = hex('e');
    ...
    let arms = [0_usize, 1].map(|index| {
        ...
        let ledger = crate::proof_ledger::VerifiedLedgerArm {
            ...
            attempt_index_prefix_root_sha256: roots[index].clone(),
            run_manifest_sha256: manifest_shas[index].clone(),
        };
        let receipt = crate::ArmReceipt {
            schema_version: 1,
            pair_id: pair_id.clone(),
            frozen_run_context_sha256: frozen_sha256.clone(),
            execution_context_sha256: execution_context_sha256.clone(),
            run_ordinal: ordinal,
            condition,
            first_condition: conditions[0],
            second_condition: conditions[1],
            attempt_index_file_sha256: ledger.attempt_index_prefix_sha256.clone(),
            attempt_index_merkle_root: roots[index].clone(),
            global_attempt_start_inclusive: start,
            global_attempt_end_exclusive: start + 1,
            attempt_count: 1,
            completion_count: 1,
            failure_count: 0,
            timeout_count: 0,
            ...
            run_manifest_sha256: Some(manifest_shas[index].clone()),
            receipt_sha256: receipt_shas[index].clone(),
        };
        LiveArmAuthority {
            run_ordinal: ordinal,
            condition,
            core_run_ordinal: ordinal,
            core_condition: condition,
            core_manifest_sha256: manifest_shas[index].clone(),
            core_usage: usage.clone(),
            core_raw_response_count: 1,
            manifest_sha256: manifest_shas[index].clone(),
            manifest_pair_id: pair_id.clone(),
            manifest_frozen_sha256: frozen_sha256.clone(),
            manifest_execution_sha256: execution_context_sha256.clone(),
            manifest_execution_mode: ExecutionMode::Live,
            manifest_fork_sha: "a".repeat(40),
            manifest_case_sha256: hex('1'),
            manifest_attempt_ledger_sha256: ledger.attempt_index_prefix_sha256.clone(),
            manifest_attempt_index_root_sha256: ledger.attempt_index_prefix_root_sha256.clone(),
            provider_label: "approved-provider".into(),
            model_label: "approved-model".into(),
            actual_model_revision: "approved-model-revision".into(),
            usage_scope: "completeNativeThreadTree".into(),
            usage: usage.clone(),
            provider_request_attempt_count: 1,
            provider_completed_response_count: 1,
            raw_response_count: 1,
            authorized_per_run_fen: 400,
            max_provider_request_attempts: 2,
            max_total_tokens: 1_000,
            max_elapsed_seconds: 60,
            elapsed_ms: 30_000,
            mode_evidence: ModeEvidence::Replay {
                fixture_set_sha256: hex('0'),
            },
            ledger,
            receipt,
            receipt_sha256: receipt_shas[index].clone(),
    ...
    let pair_receipt = crate::PairReceipt {
        ...
        frozen_run_context_sha256: frozen_sha256.clone(),
        execution_context_sha256: execution_context_sha256.clone(),
        first_arm_receipt_sha256: receipt_shas[0].clone(),
        second_arm_receipt_sha256: receipt_shas[1].clone(),
        first_run_manifest_sha256: Some(manifest_shas[0].clone()),
        second_run_manifest_sha256: Some(manifest_shas[1].clone()),
        total_attempt_count: 2,
        total_completion_count: 2,
        total_failure_count: 0,
        total_timeout_count: 0,
        arm_order_commitment: arm_order_commitment.clone(),
        final_attempt_index_root: roots[1].clone(),
        finished_at: FINISHED.into(),
    };
    let data = LiveAuthorityData {
        private_root: "/synthetic/private".into(),
        pair_id,
        ...
        provider_endpoint_commitment: Some(hex('3')),
        pair_receipt,
        pair_receipt_sha256: hex('4'),
        attempt_ledger_sha256,
        attempt_index_root_sha256: roots[1].clone(),
        attestation,
        fixed_max_output_tokens: 4_096,
        fixed_max_provider_request_attempts: 2,
        fixed_max_total_tokens: 1_000,
        fixed_max_elapsed_seconds: 60,
        arms,
    };
    let boundary = SyntheticCostBoundary {
        documents: [Vec::new(), rate_card, billing_policy, fx_policy, budget],
        response_usage,
        published_receipt: None,
    };
    let mut input = SyntheticLiveCostAuthority {
        data,
        boundary: std::rc::Rc::new(std::cell::RefCell::new(boundary)),
    };
    rebind(&mut input);
    input
}
...
fn synthetic_manifest_bytes(input: &SyntheticLiveCostAuthority) -> [Vec<u8>; 2] {
    input.data.arms.each_ref().map(|arm| {
        crate::jcs::canonicalize_value(
            &serde_json::to_value(crate::RunManifest {
                schema_version: 1,
                pair_id: input.data.pair_id.clone(),
                ...
                run_ordinal: arm.run_ordinal,
                condition: arm.condition,
                fork_sha: input.data.fork_sha.clone(),
                case_sha256: arm.manifest_case_sha256.clone(),
                source_materials_sha256: hex('0'),
                prompt_sha256: hex('0'),
                additional_context_sha256: hex('0'),
                output_schema_sha256: hex('0'),
                thread_start_request_sha256: hex('0'),
                turn_start_request_sha256: hex('0'),
                shared_config_sha256: hex('0'),
                effective_config_sha256: hex('0'),
                config_layers_sha256: hex('0'),
                native_skill_sha256: Some(hex('0')),
                pre_skill_catalog_sha256: hex('0'),
                post_skill_catalog_sha256: hex('0'),
                normalized_base_catalog_sha256: hex('0'),
                skill_use_evidence_sha256: (arm.condition == EvaluationCondition::Candidate)
                    .then(|| hex('0')),
                codex_binary_sha256: hex('0'),
                evaluator_binary_sha256: hex('0'),
                broker_component_sha256: hex('0'),
                first_root_provider_request_commitment: hex('0'),
                normalized_first_root_request_commitment: hex('0'),
                normalized_first_root_base_commitment: hex('0'),
                first_root_treatment_diff_commitment: (arm.condition
                    == EvaluationCondition::Candidate)
                    .then(|| hex('0')),
                app_server_transcript_sha256: hex('0'),
                broker_attempt_ledger_sha256: arm.manifest_attempt_ledger_sha256.clone(),
                attempt_index_root_sha256: arm.manifest_attempt_index_root_sha256.clone(),
                postprocess_evidence_index_sha256: hex('0'),
                content_package_sha256: hex('0'),
                root_thread_id: format!("root-thread-{}", arm.run_ordinal),
                root_turn_id: format!("root-turn-{}", arm.run_ordinal),
                session_id: format!("session-{}", arm.run_ordinal),
                ...
                raw_response_count: arm.raw_response_count,
                usage_scope: arm.usage_scope.clone(),
                usage: arm.usage.clone(),
                model_label: arm.model_label.clone(),
                actual_model_revision: arm.actual_model_revision.clone(),
                deployment_or_fingerprint_commitment: input
                    .data
                    .provider_endpoint_commitment
                    .clone(),
                provider_label: arm.provider_label.clone(),
                ...
                max_provider_request_attempts: arm.max_provider_request_attempts,
                max_total_tokens: arm.max_total_tokens,
                max_elapsed_seconds: arm.max_elapsed_seconds,
                elapsed_ms: arm.elapsed_ms,
                tree_closed: true,
                execution_mode: crate::ExecutionMode::Live,
                mode_evidence: arm.mode_evidence.clone(),
            })
            .unwrap(),
        )
        .unwrap()
...
fn mutate(mut input: SyntheticLiveCostAuthority, mutation: Mutation) -> SyntheticLiveCostAuthority {
    match mutation {
        Mutation::UsageDrift => {
            ...
            let mut records: Vec<SyntheticResponseUsage> =
                serde_json::from_slice(&side.bytes).unwrap();
            records[0].usage.total_tokens += 1;
            side.bytes = serde_json::to_vec(&records).unwrap();
            side.sha256 = sha256(&side.bytes);
        }
        Mutation::AttestationRawUnknown => {
            edit_document(&input, ATTESTATION, |value| value["unknown"] = true.into());
            refresh_evidence(&mut input);
            return input;
        }
        ...
        Mutation::AttestationBillingCommitment => {
            input.data.attestation.billing_policy_commitment = hex('0')
        }
        Mutation::AttestationFxCommitment => input.data.attestation.fx_policy_sha256 = hex('0'),
        Mutation::AttestationBudgetCommitment => {
            input.data.attestation.provider_budget_evidence_sha256 = hex('0')
        }
        ...
        Mutation::DuplicateCandidate | Mutation::MissingCandidate => {
            let condition = if matches!(mutation, Mutation::DuplicateCandidate) {
                EvaluationCondition::Candidate
            } else {
            ...
            for arm in &mut input.data.arms {
                ...
                arm.core_condition = condition;
                arm.ledger.condition = condition;
                arm.receipt.condition = condition;
                arm.receipt.first_condition = condition;
                arm.receipt.second_condition = condition;
            }
        }
        Mutation::Endpoint => input.data.provider_endpoint_commitment = Some(hex('2')),
        Mutation::Order => input.data.arm_order_commitment = hex('2'),
        Mutation::FutureSigned => {
            input.data.attestation.signed_at = "2026-08-30T09:31:00.000Z".into()
        }
        ...
        Mutation::TargetEvidence => {
            input
                .data
                .attestation
                .target_provider_model_evidence_commitment = hex('2');
        }
        ...
        Mutation::AttemptCap => {
            ...
            arm.provider_request_attempt_count = 2;
            arm.ledger.provider_request_attempt_count = 2;
            arm.receipt.attempt_count = 2;
            arm.ledger.global_end_exclusive = 2;
            arm.receipt.global_attempt_end_exclusive = 2;
            input.data.pair_receipt.total_attempt_count = 3;
        }
        Mutation::Deadline => {
            input.data.arms[1].receipt.sealed_at = "2026-08-30T10:31:00.000Z".into()
        }
        Mutation::BudgetAbove => {
            edit_document(&input, BUDGET, |value| {
                value["prepaidOrHardLimitFen"] = 801.into()
            });
        }
        ...
        Mutation::RateAfterStart => {
            edit_document(&input, RATE, |value| {
                value["effectiveAt"] = Value::String("2026-08-30T09:31:00.000Z".into())
            });
            input.data.attestation.rate_effective_at = "2026-08-30T09:31:00.000Z".into();
        }
        Mutation::BillingAfterStart => {
            edit_document(&input, BILLING, |value| {
                value["effectiveAt"] = Value::String("2026-08-30T09:31:00.000Z".into())
            });
        }
        Mutation::FxAfterStart => {
            edit_document(&input, FX, |value| {
                value["effectiveAt"] = Value::String("2026-08-30T09:31:00.000Z".into())
            });
        }
        Mutation::RateExpiryEqual => {
            edit_document(&input, RATE, |value| {
                value["expiresAt"] = Value::String(FINISHED.into())
            });
        }
        Mutation::RateExpired => {
            edit_document(&input, RATE, |value| {
                value["expiresAt"] = Value::String("2026-08-30T09:59:00.000Z".into())
            });
        }
        Mutation::BudgetWindow => {
            edit_document(&input, BUDGET, |value| {
                value["validFrom"] = Value::String("2026-08-30T09:31:00.000Z".into())
            });
        }
        ...
        Mutation::SafeIntegerMaximum => {
            ...
            edit_document(&input, BUDGET, |value| {
                value["prepaidOrHardLimitFen"] = 9_007_199_254_740_991_u64.into()
            });
        }
    }
    if matches!(
        mutation,
        Mutation::AttestationRateCommitment
            | Mutation::AttestationBillingCommitment
            | Mutation::AttestationFxCommitment
            | Mutation::AttestationBudgetCommitment
    ) {
...
rejection_test!(
    cost_authority_rejects_rate_card_effective_time_drift,
    RateEffectiveMismatch,
    "rate effective"
);
rejection_test!(
    cost_authority_rejects_rate_expiry_equal_to_finish,
    RateExpiryEqual,
    "rate expiry"
);
rejection_test!(
    cost_authority_rejects_rate_expired_during_run,
    RateExpired,
    "rate expiry"
);
rejection_test!(
    cost_authority_rejects_invalid_budget_validity_window,
    BudgetWindow,
    "budget validity"
);
...
fn cost_authority_rejects_manifest_attempt_or_pair_final_root_drift() {
    for mutation in [
        Mutation::ManifestLedger,
        Mutation::ManifestRoot,
        Mutation::PairFinalRoot,
    ] {
...
fn clock(value: &str) -> FixedClock {
    FixedClock {
        value: Ok(DateTime::parse_from_rfc3339(value)
            .unwrap()
            .with_timezone(&Utc)),
        calls: Cell::new(0),
    }
...
fn commit_error(
    ...
    let before = state(boundary.as_ref()).borrow().clone();
    let authority =
        prepare_synthetic_live_cost_authority(input).unwrap_or_else(|error| panic!("{error:#}"));
    let error =
...
fn cost_authority_rejects_calculation_before_finish() {
    ...
    assert!(
        commit_error(synthetic_input(), None, &clock)
            .to_string()
            .contains("before finish")
    );
}
...
fn cost_authority_rejects_supplier_issue_time_after_calculation() {
    let statement =
        supplier(|value| value["issuedAt"] = Value::String("2026-08-30T10:02:00.000Z".into()));
    let clock = clock("2026-08-30T10:01:00.000Z");
    assert!(
        commit_error(synthetic_input(), Some(&statement), &clock)
            .to_string()
            .contains("supplier issue")
    );
}
...
fn cost_authority_rejects_supplier_issue_time_before_selected_seal() {
    let statement =
        supplier(|value| value["issuedAt"] = Value::String("2026-08-30T09:49:59.999Z".into()));
    let clock = clock("2026-08-30T10:01:00.000Z");
    assert!(
        commit_error(synthetic_input(), Some(&statement), &clock)
            .to_string()
...
fn cost_authority_rejects_supplier_authority_mismatch() {
    for field in [
        ...
        let statement = supplier(|value| {
            value[field] = Value::String(match field {
...
fn cost_authority_rejects_emitted_integer_above_jcs_safe_boundary() {
    ...
    assert!(
        commit_error(
            mutate(synthetic_input(), Mutation::UnsafeInteger),
            None,
            &clock
        )
        .to_string()
...
fn cost_authority_accepts_exact_jcs_safe_integer_boundary() {
    ...
    assert_eq!(
        receipt.ceilings.prepaid_or_hard_limit_fen,
...
fn cost_authority_normalizes_submillisecond_clock_exactly_once() {
    ...
    let receipt =
        commit_cost_receipt(&authority, EvaluationCondition::Candidate, None, &clock).unwrap();
...
fn cost_authority_builds_exact_happy_receipt_without_supplier() {
    ...
    assert_eq!(
        receipt_sha(&receipt),
        "738400372f6cbba6d70e076b6db9eb669304e3e66da79180b99d2b5d1049109d"
    );
    assert_eq!(
        state(boundary.as_ref()).borrow().published_receipt.as_ref(),
        Some(&receipt)
...
fn cost_authority_builds_candidate_first_committed_receipt() {
    let input =
        synthetic_input_with_order([EvaluationCondition::Candidate, EvaluationCondition::Generic]);
    let boundary = input.boundary.clone();
    let before = state(boundary.as_ref()).borrow().clone();
    let authority = prepare_synthetic_live_cost_authority(input).unwrap();
    assert_eq!(*state(boundary.as_ref()).borrow(), before);
...
fn cost_authority_builds_exact_happy_receipt_with_over_ceiling_supplier_actual() {
    ...
    assert_eq!(
        receipt_sha(&receipt),
        "e4197bede0a13e2df6609b80e77b08e3bc7d6627d0f178e783151881a678a210"
    );
    assert_eq!(
        state(boundary.as_ref()).borrow().published_receipt.as_ref(),
        Some(&receipt)
...
fn transaction_world(supplier: Option<&[u8]>, covered: bool) -> CostTransactionWorld {
    ...
    if let Some(bytes) = supplier {
        ...
        if covered {
            let old_root = crate::private_inventory::verify_private_inventory(&root).unwrap_err();
...
fn cost_binding_world_for(
    ...
    let supplier =
        with_over_ceiling_supplier.then(|| supplier_raw(|value| value["actualFen"] = 450.into()));
...
fn run_transaction(
    ...
    run_transaction_for_condition(world, EvaluationCondition::Candidate, supplier, clock, hook)
}
...
fn cost_receipt_transaction_publishes_pending_supplier_state_a() {
    ...
    assert_eq!(
        receipt(&world).supplier_statement_sha256,
        Some(sha256(&bytes))
    );
    crate::private_inventory::verify_private_inventory(&world.root).unwrap();
}
...
fn cost_receipt_transaction_rejects_changed_or_wrong_identity_supplier_before_append() {
    for field in ["pairId", "providerLabel"] {
        let bytes = supplier_raw(|value| {
            value[field] = Value::String(if field == "pairId" {
                hex('e')
            } else {
        ...
        let before = inventory_bytes(&world.root);
        let error =
            run_transaction(&world, true, &transaction_clock(), &mut |_| Ok(())).unwrap_err();
...
fn cost_receipt_transaction_before_supplier_append_leaves_pending_state() {
    ...
    let mut hook = |checkpoint| match checkpoint {
        CostReceiptCheckpoint::BeforeSupplierInventoryAppend => {
            anyhow::bail!("stop before supplier append")
        }
        _ => Ok(()),
    };
    let error = run_transaction(&world, true, &transaction_clock(), &mut hook).unwrap_err();
...
fn cost_receipt_transaction_after_supplier_append_resumes_as_state_b() {
    ...
    let world = transaction_world(Some(&bytes), false);
    let mut hook = |checkpoint| match checkpoint {
        CostReceiptCheckpoint::AfterSupplierInventoryAppend => {
            anyhow::bail!("stop after supplier append")
        }
        _ => Ok(()),
    };
    let error = run_transaction(&world, true, &transaction_clock(), &mut hook).unwrap_err();
...
fn cost_receipt_transaction_after_receipt_create_rerun_fails_closed() {
    let world = transaction_world(None, false);
    let mut hook = |checkpoint| match checkpoint {
        CostReceiptCheckpoint::AfterReceiptCreateBeforeInventoryAppend => {
            anyhow::bail!("stop after receipt create")
        }
        _ => Ok(()),
    };
    let error = run_transaction(&world, false, &transaction_clock(), &mut hook).unwrap_err();
...
fn cost_receipt_transaction_after_receipt_append_has_valid_inventory() {
    let world = transaction_world(None, false);
    let mut hook = |checkpoint| match checkpoint {
        CostReceiptCheckpoint::AfterReceiptInventoryAppend => {
            anyhow::bail!("stop after receipt append")
        }
        _ => Ok(()),
    };
    let error = run_transaction(&world, false, &transaction_clock(), &mut hook).unwrap_err();
