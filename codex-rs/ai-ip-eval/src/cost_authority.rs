...
pub(crate) fn retain_supplier_statement(bytes: &[u8]) -> Result<RetainedSupplierStatement> {
    use sha2::Digest;
...
pub(crate) fn prepare_live_cost_authority(
    ...
    prepare_authority(
        ...
        LiveAuthoritySource::Production(Box::new(core)),
    )
}
#[cfg(test)]
pub(crate) fn prepare_synthetic_live_cost_authority(
    ...
    prepare_authority(
        data,
        &documents[0],
        [&documents[1], &documents[2], &documents[3], &documents[4]].map(Vec::as_slice),
        source,
    )
}
...
fn calculate_cost_receipt_at(
    ...
    if selected.next().is_some() {
        bail!("selected receipt condition is ambiguous")
    }
    ...
    let receipt = CostReceiptV1 {
        schema_version: 1,
        pair_id: authority.data.pair_id.clone(),
        ...
        execution_manifest_sha256: arm.manifest_sha256.clone(),
        broker_receipt_sha256: arm.receipt_sha256.clone(),
        pair_receipt_sha256: authority.data.pair_receipt_sha256.clone(),
        attempt_ledger_sha256: authority.data.attempt_ledger_sha256.clone(),
        condition,
        ...
        attempt_range: AttemptRangeV1 {
            start_inclusive: arm.ledger.global_start_inclusive,
            end_exclusive: arm.ledger.global_end_exclusive,
        },
        provider_label: arm.provider_label.clone(),
        actual_model_revision: arm.actual_model_revision.clone(),
        rate_card_sha256: authority.inputs.rate_card_sha256.clone(),
        billing_policy_commitment: authority.inputs.billing_policy_commitment.clone(),
        fx_policy_sha256: authority.inputs.fx_policy_sha256.clone(),
        provider_budget_evidence_sha256: authority.inputs.provider_budget_evidence_sha256.clone(),
        ...
        usage_scope: "rootSessionTree".into(),
        usage: arm.usage.clone(),
        currency: "CNY".into(),
        rate_effective_at: authority.inputs.rate_card.effective_at.clone(),
        fx: FxReceiptV1 {
            mode: "notApplicable".into(),
            numerator: 1,
            denominator: 1,
        },
        ceilings: CostCeilingsV1 {
            approved_per_run_fen: attestation.approved_per_run_fen,
            approved_total_fen: attestation.approved_total_fen,
            ...
            max_total_tokens: attestation.max_total_tokens_per_run,
            max_elapsed_seconds: attestation.max_elapsed_seconds_per_run,
        },
        calculated_at: calculated_at.to_string(),
        calculation: CostCalculationV1 {
            rate_unit: "fenPerMillionTokens".into(),
            rounding: "ceilingToFen".into(),
            reasoning_tokens_billed_separately: false,
        },
        estimated_fen: calculated.estimated_fen,
        supplier_statement_sha256: statement.map(|value| value.sha256.clone()),
        supplier_actual_fen: calculated.supplier_actual_fen,
        charged_fen: calculated.charged_fen,
        within_ceilings: calculated.within_ceilings,
    };
    ...
    if validated != receipt {
        bail!("canonical receipt validator changed the typed value")
    }
    Ok(validated)
}
...
fn production_parts(core: &PairEvidenceCore) -> Result<(LiveAuthorityData, Vec<u8>, [Vec<u8>; 4])> {
    ...
    let projection = core
        .cost_projection
        .as_ref()
        .ok_or_else(|| anyhow::anyhow!("Live cost projection is absent"))?;
    if core.mode != projection.execution_mode {
        bail!("core and cost projection modes differ")
    }
    ...
    let arms = [0_usize, 1].map(|index| {
        ...
        LiveArmAuthority {
            ...
            core_run_ordinal: core_arm.run_ordinal,
            core_condition: core_arm.condition,
            core_manifest_sha256: core_arm.run_manifest_raw_sha256.clone(),
            core_usage: core_arm.usage.clone(),
            core_raw_response_count: core_arm.raw_response_count,
            manifest_sha256: projected.manifest_sha256.clone(),
            manifest_pair_id: manifest.pair_id.clone(),
            manifest_frozen_sha256: manifest.frozen_run_context_sha256.clone(),
            manifest_execution_sha256: manifest.execution_context_sha256.clone(),
            manifest_execution_mode: manifest.execution_mode,
            ...
            manifest_fork_sha: manifest.fork_sha.clone(),
            manifest_case_sha256: manifest.case_sha256.clone(),
            ...
            usage_scope: manifest.usage_scope.clone(),
            ...
            authorized_per_run_fen: manifest.authorized_evaluation_run_cost_fen,
            max_provider_request_attempts: manifest.max_provider_request_attempts,
            max_total_tokens: manifest.max_total_tokens,
            max_elapsed_seconds: manifest.max_elapsed_seconds,
            elapsed_ms: manifest.elapsed_ms,
            mode_evidence: projected.mode_evidence.clone(),
            ledger: projected.ledger.clone(),
            receipt: projected.arm_receipt.clone(),
            receipt_sha256: projected.arm_receipt_sha256.clone(),
        }
    });
    let data = LiveAuthorityData {
        private_root: core
            .private_root
            .to_str()
            .ok_or_else(|| anyhow::anyhow!("private root is not UTF-8"))?
            .into(),
        pair_id: core.pair_id.clone(),
        fork_sha: core.fork_sha.clone(),
        frozen_sha256: core.frozen_run_context_sha256.clone(),
        execution_mode: projection.execution_mode,
        execution_context_sha256: projection.execution_context_sha256.clone(),
        started_at: projection.started_at.clone(),
        deadline: projection.deadline.clone(),
        arm_order_commitment: projection.arm_order_commitment.clone(),
        provider_endpoint_commitment: projection.provider_endpoint_commitment.clone(),
        pair_receipt: projection.pair_receipt.clone(),
        pair_receipt_sha256: projection.pair_receipt_sha256.clone(),
        ...
        attestation: retained.attestation.clone(),
        fixed_max_output_tokens: frozen.max_output_tokens(),
        fixed_max_provider_request_attempts: frozen.max_attempts_per_arm(),
        fixed_max_total_tokens: frozen.max_total_tokens_per_run(),
        fixed_max_elapsed_seconds: frozen.max_elapsed_seconds_per_run(),
        arms,
    };
    let raw: [Vec<u8>; 4] = [
        "rateCard",
    ...
    .into_iter()
    .map(|name| frozen.artifact_bytes(name))
    .collect::<Result<Vec<_>>>()?
    .try_into()
    .map_err(|_| anyhow::anyhow!("missing frozen cost input"))?;
...
fn validate_parts(
    ...
    attestation_raw: &[u8],
    raw: [&[u8]; 4],
) -> Result<VerifiedCostInputs> {
    let attestation = crate::contracts::FrozenContracts::load()?
        .validate_native_attestation(attestation_raw)
        .map_err(|error| anyhow::anyhow!("attestation contract rejected exact bytes: {error}"))?;
    if attestation != data.attestation {
        bail!("attestation raw bytes differ from typed authority")
    }
    let inputs = crate::cost_contracts::FrozenCostContracts::load()?
        .validate_inputs(raw[0], raw[1], raw[2], raw[3])?;
...
impl VerifiedLiveCostAuthority {
    pub(crate) fn reverify(&self) -> Result<()> {
        let inputs = match &self.source {
            LiveAuthoritySource::Production(core) => {
                ...
                let (data, attestation, raw) = production_parts(core)?;
                if data != self.data {
                    bail!("Live cost authority changed during reverify")
                }
        ...
        if !same_inputs(&inputs, &self.inputs)? {
            bail!("Live cost inputs changed during reverify")
        }
        Ok(())
    }
    ...
    pub(crate) fn transaction_binding(&self) -> (&str, &str, &str) {
        (
            &self.data.pair_id,
            &self.data.frozen_sha256,
            &self.data.private_root,
        )
    }
...
fn same_inputs(left: &VerifiedCostInputs, right: &VerifiedCostInputs) -> Result<bool> {
    Ok(left.rate_card_sha256 == right.rate_card_sha256
        && left.billing_policy_commitment == right.billing_policy_commitment
        && left.fx_policy_sha256 == right.fx_policy_sha256
        && left.provider_budget_evidence_sha256 == right.provider_budget_evidence_sha256
        && serde_json::to_value((
            &left.rate_card,
            &left.billing_policy,
            &left.fx_policy,
...
fn validate_authority(
    ...
    if !live_mode_matches {
        bail!("Live mode evidence is inconsistent")
    }
    let ModeEvidence::Live {
        ...
        rate_card_sha256,
        billing_policy_sha256,
        fx_policy_sha256,
        authorized_pair_cost_fen,
        retention_deadline,
    } = &first.mode_evidence
    else {
    ...
    if !provider_role_matches {
        bail!("provider role differs from attestation")
    }
    ...
    if !cost_commitments_match {
        bail!("attestation cost input commitments differ")
    }
    ...
    if !order_matches {
        bail!("arm order commitment differs")
    }
    if !has_exact_condition_set(first.condition, second.condition) {
        bail!("authority does not contain exact ordered conditions")
    }
    ...
    let signed = parse_millis(&attestation.signed_at)?;
    let started = parse_millis(&data.started_at)?;
    let finished = parse_millis(&data.pair_receipt.finished_at)?;
    let deadline = parse_millis(&data.deadline)?;
    let retention = parse_millis(&attestation.retention_deadline)?;
    let attestation_timeline_valid =
        candidate_frozen < case_selected && case_selected <= signed && signed <= started;
    if !attestation_timeline_valid {
        bail!("attestation signedAt timeline is invalid")
    }
    if started >= finished || finished > deadline {
        bail!("execution deadline timeline is invalid")
    }
    if finished >= retention {
        bail!("retention timeline is inverted or expired")
    }
    if inputs.rate_card.effective_at != attestation.rate_effective_at {
        bail!("rate effective time differs from attestation")
    }
    for effective in [
        &inputs.rate_card.effective_at,
        &inputs.billing_policy.effective_at,
        &inputs.fx_policy.effective_at,
    ] {
        if parse_millis(effective)? > started {
            bail!("cost policy is effective after run start")
        }
    }
    let rate_expired = inputs.rate_card.expires_at.as_ref().is_some_and(|value| {
        parse_millis(value)
            .map(|time| time <= finished)
            .unwrap_or(true)
    });
    if rate_expired {
        bail!("rate expiry does not strictly follow finish")
    }
    ...
    if !approval_matches {
        bail!("candidate, private root, or approved total authority differs")
    }
    if data.fixed_max_output_tokens != attestation.max_output_tokens_per_request {
        bail!("fixed max output tokens differ")
    }
    ...
    if !frozen_limits_match {
        bail!("frozen hard gates differ from attestation")
    }
    ...
    if !provider_matches {
        bail!("provider or model identity differs")
    }
    let currencies = (
        inputs.rate_card.currency.as_str(),
        inputs.billing_policy.currency.as_str(),
        inputs.budget.currency.as_str(),
        attestation.rate_currency.as_str(),
    );
    let fx = (
        inputs.fx_policy.mode.as_str(),
        inputs.fx_policy.source_currency.as_str(),
        inputs.fx_policy.target_currency.as_str(),
        inputs.fx_policy.numerator,
        inputs.fx_policy.denominator,
    );
    if currencies != ("CNY", "CNY", "CNY", "CNY") || fx != ("notApplicable", "CNY", "CNY", 1, 1) {
        bail!("cost currency or FX authority differs")
    }
    ...
    if !final_root_matches {
        bail!("pair final attempt commitment differs from ordered second arm")
    }
    if data.attempt_ledger_sha256 != second.ledger.attempt_index_prefix_sha256 {
        bail!("full attempt ledger commitment differs from ordered second arm")
    }
    ...
    let pair_counts_match = first
        .receipt
        .attempt_count
        .checked_add(second.receipt.attempt_count)
        == Some(data.pair_receipt.total_attempt_count)
        && first
            .receipt
            .completion_count
            .checked_add(second.receipt.completion_count)
        ...
        && first
            .receipt
            .timeout_count
            .checked_add(second.receipt.timeout_count)
            == Some(data.pair_receipt.total_timeout_count);
    if !pair_identity_matches || !pair_counts_match {
        bail!("pair receipt identity or counts differ")
    }
    for (index, arm) in data.arms.iter().enumerate() {
        let ordinal = u8::try_from(index + 1)?;
        let ordinal_matches = arm.run_ordinal == ordinal
            && arm.core_run_ordinal == ordinal
            && arm.receipt.run_ordinal == ordinal
            && arm.condition == arm.core_condition
            && arm.condition == arm.ledger.condition
            && arm.condition == arm.receipt.condition;
        if !ordinal_matches {
            bail!("arm ordinal or condition differs")
        }
        let previous_receipt_matches = (index == 0
            && arm.receipt.previous_arm_receipt_sha256.is_none())
            || (index == 1
                && arm.receipt.previous_arm_receipt_sha256.as_deref()
        ...
        let receipt_identity_matches = arm.receipt.pair_id == data.pair_id
            ...
            && arm.receipt.second_condition == second.condition
            && previous_receipt_matches;
        if !receipt_identity_matches {
            bail!("arm receipt pair or order identity differs")
        }
        ...
        if !manifest_identity_matches {
            bail!("manifest pair or candidate identity differs")
        }
        let usage_matches = arm.usage_scope == "completeNativeThreadTree"
            && arm.usage == arm.core_usage
            && arm.usage == arm.ledger.usage;
        if !usage_matches {
            bail!("broker/App Server response usage multiset differs")
        }
        ...
        if !document_sha_matches {
            bail!("manifest or arm receipt SHA differs")
        }
        let attempt_commitments_match = arm.manifest_attempt_ledger_sha256
            ...
            && arm.manifest_attempt_index_root_sha256
                == arm.ledger.attempt_index_prefix_root_sha256
            && arm.manifest_attempt_index_root_sha256 == arm.receipt.attempt_index_merkle_root;
        if !attempt_commitments_match {
            bail!("manifest attempt commitment differs from ledger or receipt")
        }
        let response_counts_match = arm.provider_request_attempt_count
            == arm.ledger.provider_request_attempt_count
            && arm.provider_completed_response_count
                == arm.ledger.provider_completed_response_count
        ...
        if !response_counts_match {
            bail!("arm response counts differ")
        }
        let sealed_counts_match = arm.receipt.in_flight == 0
            && arm.receipt.failure_count == 0
            && arm.receipt.timeout_count == 0
            && arm
                .receipt
                .global_attempt_end_exclusive
                .checked_sub(arm.receipt.global_attempt_start_inclusive)
                == Some(arm.receipt.attempt_count);
        if !sealed_counts_match {
            bail!("sealed receipt invariants differ")
        }
        let range_matches = arm.receipt.global_attempt_start_inclusive
            == arm.ledger.global_start_inclusive
        ...
        if !hard_gates_match {
            bail!("manifest hard gates differ")
        }
        if arm.provider_request_attempt_count > attestation.max_provider_request_attempts_per_run {
            bail!("provider request attempt cap exceeded")
        }
        let elapsed_limit = u128::from(attestation.max_elapsed_seconds_per_run)
            .checked_mul(1_000)
            .ok_or_else(|| anyhow::anyhow!("elapsed hard gate overflow"))?;
        if arm.elapsed_ms > elapsed_limit || parse_millis(&arm.receipt.sealed_at)? > deadline {
            bail!("arm deadline or elapsed hard gate exceeded")
        }
    }
    if parse_millis(&first.receipt.sealed_at)? > parse_millis(&second.receipt.sealed_at)?
        || parse_millis(&second.receipt.sealed_at)? > finished
    {
        bail!("sealed receipt timestamp order differs")
    }
    Ok(())
}
...
pub(crate) fn project_verified_native_cost(
    ...
    let project_arm = |manifest: &ExactDocument<RunManifest>, ledger, arm_receipt: ArmReceipt| {
        CostArmProjection {
