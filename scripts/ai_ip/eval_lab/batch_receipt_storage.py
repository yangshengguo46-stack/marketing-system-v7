...
def bind_child(
    parent: BoundDirectory, name: str, expected: tuple[int, int]
) -> BoundDirectory:
    ...
    try:
        ...
        state = os.fstat(descriptor)
        if (
            (state.st_dev, state.st_ino) != expected
            or not stat.S_ISDIR(state.st_mode)
            or stat.S_IMODE(state.st_mode) != 0o700
            or state.st_uid != os.getuid()
        ):
            raise SecureStorageError("private evidence child identity differs")
        parent.verify()
        return BoundDirectory(parent.path / name, descriptor, expected)
...
def read_entry(directory: BoundDirectory, name: str, limit: int) -> bytes:
    if not name or "/" in name or "\\" in name:
        raise SecureStorageError("invalid private evidence file name")
    directory.verify()
    descriptor = -1
