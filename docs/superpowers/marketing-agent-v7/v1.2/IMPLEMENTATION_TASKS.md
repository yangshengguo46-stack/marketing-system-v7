# 可交给 Coding Agent 的施工任务单 · v1.2 业务融合版
...
先读 `BUSINESS_ARCHITECTURE.md`（内容也已合并进 `FINAL_SPEC.md` 第B章）。下面业务任务依附现有WP，不创建另一个项目或运行时：

| 业务任务 | 并入工程包 | 具体交付 |
|---|---|---|
| B00 现有资产核对 | WP00 | 课程与方法来源清单、candidate状态、阴性结果、旧附件保留/替换项 |
| B01 出厂专业立场与方法 | WP00–03 | `ai-ip-assets/business/{lead-context.md,marketing-judgment.md,manifest.json}` 有真实内容，不是空文件 |
| B02 业务对象与工作认知 | WP01–03 | MarketingContext、OpportunityHypothesis、CreativeCandidate、InterventionProposal 的可选载荷与来源引用；复用work/artifact/claim |
| B03 首轮完整作品 | WP04 | 空客户记忆下用真实国内profile完成约定作品；专业选择由Lead承担 |
| B04 正文优先的业务审核 | WP05 | 剧情/事实/营销因果分开；任务适配；具体返工和原稿保留 |
| B05 系统增益与删减 | WP06 | 同模型正常通用Agent、工程组、工程+业务组的轻量比较；额外资料与成本单列 |
| B06 外部价值闭环 | WP07–09 | 作品/证明/Offer/渠道动作与授权、回执、结果相连，不能变成第二编剧总控 |

**建议新增文件（不是当前已实现的API）：**

- `codex-rs/ai-ip-domain/src/{marketing_context.rs,creative_candidate.rs,intervention.rs}`。
- `codex-rs/ai-ip-runtime/src/business_context.rs`：只做装配与投影，不加前置模型路由。
- 继续扩展原计划的work、artifact、evidence、world_state、review接口，不另造MarketingState数据库。
- 保留 `ContentPackage` v1和旧JSON读取；有`deny_unknown_fields`的旧对象不得原地增字段破坏兼容。

**业务方法内容：**采用第B章M1–M6的目标规格。现有 `story-work-v1` 保留原文与试验状态，按需引用并验证，不自动宣称已上线或必经。默认专业职责可以实验性接入；生产推广必须有真实作品证据。

**关键反例测试：**无primitive也能交付证明稿；局部改稿不重做全局业务分析；市场Actor/故事人物/观看受众不混；课程案例不冒充客户事实；“作者说感人”不替正文打分；模型评价不写为传播效果；相同事件多个视角不只是换标题；用户改目标后旧候选按实际依赖复核。

**优先级：**不用先完成全部工程再写这些方法；不用先学完更多课再检验作品。WP04仍产不出可用正文时，直接定位资料、方法或模型能力，不转向扩平台和UI掩盖问题。
...
**v1.2 补充：**本包必须接入B01/B02，不能只验证搜索与存储。至少留存完整作品、对应机会/候选的必要简短说明、真实产品连接与仍未知之处。正文和作者说明分开。案例按当前任务选择，禁止强制原语、十五步或候选配额。
...
**v1.2 补充：**采用第B章第13节的任务相关业务标准，并复用“依据正文作创作取舍”。不要求所有稿件故事化，不以字段齐全代替好作品；对模型自评和真实观众结果分别记账。
...
## WP06：内容闭环业务验收与专业方法增益验证

**v1.2 补充：**方法实体和导航已前置至B01，本包不再从零整理。增加工程组与工程+业务组的贡献隔离；保留正常工具机会、原始稿、阴性结果、额外资料和实际成本。
