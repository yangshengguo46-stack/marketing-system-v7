...
The current and only executable child is `2026-08-30-06b1-proof-commitment-and-cost-binding.md` together with `2026-08-30-06b1-cost-authority-execution-annex.md`. It implements only Work Package 6B-1: common Replay/Native private-key-derived proof identity, strict retention-managed CNY pricing/budget/statement contracts, checked cost calculation, Live-only private cost receipts, and immutable manifest sidecars. Real CLI evidence remains Replay/Mock and must be refused; positive Live transactions are crate-internal synthetic tests, so the production success path remains dormant. 06B-2 owns body-free report/index and report/live-proof verifiers; 06B-3 owns crash-safe publication, shipping-boundary verification, and finalizer implementation whose real invocation remains Work Package 9; 06C owns retention closeout. This child does not run a provider, read an API key, execute the Work Package 8 approved live pair, authorize `G0/G1/G2`, set `PASS_TO_PHASE_0B`, or start product Plan 02.
