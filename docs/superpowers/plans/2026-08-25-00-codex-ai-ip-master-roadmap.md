...
The current executable child is `2026-08-28-06a-blind-review-and-score.md`. It implements only Work Package 6's first independently reviewable boundary: strict rubric/schema assets, sealed pair re-verification, three independent condition-hidden reviewer bundles, and deterministic private scoring. Cost binding, body-free public report and verifiers remain 06B; retention closeout remains 06C. This child does not run a provider, read an API key, execute the Work Package 8 approved live pair, authorize `PASS_TO_PHASE_0B`, or start product Plan 02.
