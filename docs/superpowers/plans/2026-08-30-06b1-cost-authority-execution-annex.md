...
**Task 7 Phase B mechanical commit-cap correction (2026-08-31):** the required offline `just fix -p codex-ai-ip-eval` and immediately following `just fmt` both exited zero after both scoped re-reviews were Ready Yes, but the retained annex-authorized formatter output spans 2,027 added-plus-deleted lines across 18 allowed paths. A single mechanical commit would violate the repository's hard `<800` changed-line commit gate even though no individual retained path exceeds that cap. Authorize rewriting only the provisional Phase B style and F-0007A commits by a mixed reset to the reviewed semantic-fix tip `e3a0f5d03`, preserving the exact working-tree bytes. Commit this controller amendment first, then split the already-produced formatter/fixer delta into the minimum practical path-disjoint mechanical commits, each strictly below 800 changed lines by staged numstat, without editing or reformatting those bytes. Rewrite and commit F-0007A last so it names the final mechanical commit chain and reverse rollback order. The provisional `ad21ba002` and `296fa9eca` hashes must not remain in final branch history or ledger text. Do not rerun fixer, formatter, tests, builds, provider/network actions, or change scope; the Phase B no-tests-after-formatter rule remains in force.

**Task 7 Phase B production-module line-cap correction (2026-08-31):** the fresh completion audit proved every commit cap and Phase B path fence, then correctly stopped because retained formatter/fixer output expanded `codex-rs/ai-ip-eval/src/cost_inputs.rs` from the reviewed 489 lines to 532, violating the hard `<500` production-module gate. The diff is confined to formatter layout plus clippy-equivalent method references; no business change requires retention, and splitting production logic after the frozen test/review boundary would be broader and riskier. Authorize rewriting only the final Phase B support-style and F-0007A commits: mixed-reset to the authority-core style tip `6ef3560e2`, preserve every other working-tree byte, and restore `cost_inputs.rs` exactly to its already-tested and independently reviewed `e3a0f5d03` bytes using `apply_patch`. Commit this amendment before a corrected support-style commit containing the remaining previously formatted paths, strictly below 800 changed lines. Rewrite F-0007A last with the final authority/style hashes and rollback order; provisional `881fedfce` and `3d6ba8fe8` must not remain in final history or ledger text. Prove `git diff --exit-code e3a0f5d03 -- codex-rs/ai-ip-eval/src/cost_inputs.rs`, final module/commit caps, and exact preservation of the other 17 formatter paths. Do not rerun fixer, formatter, tests, builds, provider/network actions, or alter any other source byte; the last authorized test evidence remains the semantic-fix evidence and the no-tests-after-formatter rule remains in force.

**Task 7 Phase B authority-module line-cap correction (2026-08-31):** the restarted completion audit proved `cost_inputs.rs=489` and again correctly stopped at the next hard gate: retained whole-workspace formatting expanded the explicitly excepted `codex-rs/ai-ip-eval/src/cost_authority.rs` from the reviewed 681 lines to 971, exceeding its `<700` cap through import/argument layout rather than a required business change. Authorize rewriting the Phase B commits after authority-test style tip `f9497f79d`: mixed-reset to that exact commit, preserve the remaining worktree bytes, and restore both `cost_authority.rs` and `cost_inputs.rs` exactly to their tested and independently reviewed `e3a0f5d03` bytes using `apply_patch`. Commit the pending module-cap amendments first, then collapse all remaining preserved formatter/fixer changes for the other 16 paths into one path-exact mechanical commit strictly below 800 changed lines. Rewrite F-0007A last with the final authority/style hashes and reverse rollback. Provisional `6ef3560e2`, `3f0688e76`, `411e6d63e`, and `49fa17c17` must not remain in final history or ledger text. Prove both restored production files have no diff from `e3a0f5d03`, all ordinary production modules remain `<500`, `cost_authority.rs<700`, the other 16 formatter paths remain byte-identical to the original formatter tree, and every commit remains `<800`. Do not rerun fixer, formatter, tests, builds, provider/network actions, or change any other source byte.
